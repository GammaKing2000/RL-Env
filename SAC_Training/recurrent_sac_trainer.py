...
if __name__ == "__main__":
    training_run = "1M_recurrent_sac"

    # Define paths for saving models and logs
    MODEL_DIR = os.path.join("SAC_Training/models", training_run)
    LOG_DIR = os.path.join("SAC_Training/logs", training_run)
    TENSORBOARD_LOG_DIR = "SAC_Training/logs"
    ...
    # 4. Train the model
    print("Starting Recurrent SAC training with Stable Baselines3...")
    model.learn(
        total_timesteps=200000, # 200,000
        callback=combined_callbacks
    )
    print("Recurrent SAC Training Finished.")
    ...
    model.save(os.path.join(MODEL_DIR, f"recurrent_sac_plantos_final_model-{training_run}"))
